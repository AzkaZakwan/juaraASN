...
<body class="bg-[#F3F3F3]" style="font-family: 'Poppins', sans-serif;">
    ...
    <main class="lg:ml-64 min-h-screen p-4 sm:p-6 lg:p-8 mt-16 lg:mt-0">
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <form id="editSoalForm" action="{{ route('questions.update', $question->id) }}" method="POST"
                ...
                <div>
                    ...
                    <textarea name="question_text" rows="5" {{ $isLocked ? 'readonly' : '' }}
                        class="w-full border rounded-lg p-3 mt-2" required>{{ old('question_text', $question->question_text) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="file" name="question_image" accept="image/*"
                            ...
                            {{ $isLocked ? 'bg-gray-100 cursor-not-allowed' : '' }}" {{ $isLocked ? 'disabled' : '' }}>
                    </div>
                ...
                <div>
                    ...
                    <select name="question_type" id="questionType"
                        ...
                        <option value="TWK"
                            {{ old('question_type', $question->question_type) == 'TWK' ? 'selected' : '' }}>
                            TWK
                        </option>

                        <option value="TIU"
                            {{ old('question_type', $question->question_type) == 'TIU' ? 'selected' : '' }}>
                            TIU
                        </option>

                        <option value="TKP"
                            {{ old('question_type', $question->question_type) == 'TKP' ? 'selected' : '' }}>
                            TKP
                        </option>
                        ...
                        <div class="border rounded-lg p-4">
                            ...
                            <div class="score-input hidden mt-3">
                                ...
                                <input type="number" name="scores[]"
                                    value="{{ old('scores.' . $key, $option->score) }}" min="1" max="5"
                                    class="w-full border rounded-lg p-2 mt-2 {{ $isLocked ? 'bg-gray-100 text-gray-500' : '' }}"
                                    {{ $isLocked ? 'readonly' : '' }}>
                            </div>
                ...
                <div class="flex gap-3">
                    ...
                    @if (!$isLocked)
                        <button type="submit" form="editSoalForm"
                            class="bg-[#FFA35C] text-white px-6 py-3 rounded-lg cursor-pointer">
                            Edit Soal
                        </button>
                    @else
