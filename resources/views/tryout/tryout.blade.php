    ...
    <div class="flex-1 max-w-6xl mx-auto w-full p-6 mt-4 pt-20">
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-6">
            ...
            <select id="filterPackage"
                class="w-36 border border-gray-300 rounded-xl px-4 py-3 bg-white outline-none focus:outline-none focus:ring-0 focus:border-[#FFA35C] focus:shadow-none transition">
                <option value="all">Semua</option>
                <option value="free">Gratis</option>
                <option value="premium">Premium</option>
                <option value="owned">Dibeli</option>
            </select>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                @foreach ($packages as $package)
                    <div class="package-card border-2 border-[#FFA35C] rounded-xl p-4 hover:shadow-lg transition"
                        ...
                        data-status="{{ in_array($package->id, $userPackageIds) ? 'owned' : ($package->price == 0 ? 'free' : 'premium') }}">
    ...
    function filterPackages() {
        ...
        document.querySelectorAll('.package-card').forEach(card => {
            ...
            const matchStatus =
                status === 'all' ||
                status === cardStatus ||
                (status === 'premium' && cardStatus === 'owned');
